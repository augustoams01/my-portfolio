    ...
    <section className="pb-16">
      <div className="container">
        <div className="flex justify-center">
          <p className="uppercase font-semibold tracking-widest bg-gradient-to-r from-sky-400 to-emerald-300 text-transparent bg-clip-text text-center">Resultados reais</p>
        </div>
        <h2 className="font-serif text-3xl text-center mt-6">Projetos em destaque</h2>
        <p className="text-center text-white/60 mt-4">Veja como eu transformei conceitos em experiências digitais inesqueciveis</p>
        <div className="flex flex-col gap-20 mt-10">
          {portfolioProjects.map((project) => (
            <div key={project.title} className="bg-gray-800 rounded-3xl relative z-0 overflow-hidden after:z-10 after:content-[''] after:absolute after:inset-0 after:outline-2 after:outline after:-outline-offset-2 after:rounded-3xl after:outline-white/20 px-8 pt-8 after:pointer-events-none">
              ...
              <div className="absolute inset-0 -z-10 opacity-30 bg-cover" style={{backgroundImage: `url(${BackgroundImage.src})`}}></div>
              ...
              <ul className="flex flex-col gap-4 mt-4">
                {project.results.map((result)=>(
                  <li key={result.title} className="flex gap-2 items-center text-sm text-white/50"> 
              ...
              <a href={project.link}>
                <button className="inline-flex bg-white text-gray-950 h-12 w-full rounded-xl font-semibold items-center justify-center gap-2 mt-8"><span>Visitar o site</span><ArrouwUpRight className="size-4"/></button>
              </a>
